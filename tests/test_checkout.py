...
def test_checkout_cod_creates_order_and_reserves_stock(client, customer, product, address, db):
...
def test_checkout_rejects_empty_cart(app, customer, address, db):
...
def test_checkout_clamps_quantity_when_stock_drops_before_checkout(client, customer, product, address, db):
    login(client, customer.email)
...
def test_inventory_service_rejects_reservation_over_available_stock(app, product, db):
...
def test_order_detail_blocks_other_users(client, customer, other_customer, product, address, db):
    login(client, customer.email)
...
def test_admin_order_status_transition_restores_stock_on_cancel(client, admin_user, customer, product, address, db):
...
def test_invalid_status_transition_rejected(client, admin_user, customer, product, address, db):
    login(client, customer.email)
    client.post("/cart/add", data={"product_id": product.id, "quantity": 1})
    client.post("/checkout/place", data={"address_id": address.id, "payment_method": "cod"})
    order = Order.query.filter_by(user_id=customer.id).first()
