...
def _make_product(category, name, sku, sale_price=None):
    p = Product(
...
def _order(user, product, quantity, status=OrderStatus.DELIVERED, number="ORD-1"):
    order = Order(
        user_id=user.id,
        order_number=number,
...
def test_default_homepage_sections_match_previous_behaviour(client, category):
    _make_product(category, "Plain Lamp", "LAMP-1")
    _make_product(category, "Discount Chair", "CHAIR-1", sale_price=500)
...
def test_trending_ranks_by_units_and_ignores_cancelled(client, category, customer):
    lamp = _make_product(category, "Plain Lamp", "LAMP-1")
    chair = _make_product(category, "Oak Chair", "CHAIR-1")
    _order(customer, lamp, 2, number="ORD-1")
    _order(customer, chair, 5, number="ORD-2")
    _order(customer, lamp, 50, status=OrderStatus.CANCELLED, number="ORD-3")
...
def test_admin_can_hide_reorder_and_pick_products(client, admin_user, category):
    lamp = _make_product(category, "Plain Lamp", "LAMP-1")
    chair = _make_product(category, "Oak Chair", "CHAIR-1")
    sofa = _make_product(category, "Velvet Sofa", "SOFA-1")
...
def test_manual_sale_section_skips_products_without_discount(category):
    full_price = _make_product(category, "Plain Lamp", "LAMP-1")
    discounted = _make_product(category, "Oak Chair", "CHAIR-1", sale_price=400)
